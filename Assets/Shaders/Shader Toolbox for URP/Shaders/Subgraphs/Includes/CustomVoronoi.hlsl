...
void CustomVoronoi_float(float2 UV, float AngleOffset, float CellDensity, out float DistFromCenter, out float DistFromEdge, out float RandomCell)
{
    ...
    for(int y = -1; y <= 1; ++y)
    {
        for(int x = -1; x <= 1; ++x)
        {
            ...
            float2 offset = float2(cellToCheck) - posInCell;
            float2 rand = randomVector_float(cell + cellToCheck, AngleOffset);
            float2 cellOffset = offset + rand;
...
void CustomVoronoi_half(half2 UV, half AngleOffset, half CellDensity, out half DistFromCenter, out half DistFromEdge, out half RandomCell)
{
    int2 cell = floor(UV * CellDensity);
    ...
    for (int y = -1; y <= 1; ++y)
    {
        for (int x = -1; x <= 1; ++x)
        {
            ...
            half2 offset = half2(cellToCheck) - posInCell;
            half2 rand = randomVector_half(cell + cellToCheck, AngleOffset);
            half2 cellOffset = offset + rand;
